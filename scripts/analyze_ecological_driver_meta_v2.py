...
def main():
    ...
    # Genus-level cross-species primary: one bird-access experiment per species.
    ...
    for i,r in enumerate(cross):
        kept=[x for j,x in enumerate(cross) if i!=j];z=rr_summary(kept);loo.append({'omitted':r['effect_id'],'RR':z['geometric_mean_RR']})
    cross_sum['leave_one_out_RR_min']=min(x['RR'] for x in loo);cross_sum['leave_one_out_RR_max']=max(x['RR'] for x in loo)
    cross_sum['formal_inverse_variance_pooling']=False

    # Within C. oleifera independent pollinator-service experiments: Zhang bird exclusion + Liu cage.
    ole_ids={'ECO_BIRD_OLEIFERA_FRUIT','ECO_BEE_OLEIFERA_LIU2025_CAGE'}
    ole=[r for r in e if r['effect_id'] in ole_ids];assert len(ole)==2
    ...
    # Reliability gradients: sign consistency, without combining incomparable coefficient scales.
    grad=[r for r in e if r['axis'] in {'pollinator_reliability','pollinator_reliability_gradient'}]
    expected=[]
    for r in grad:
    ...
    # Pollen limitation: one exact count-based effect, one source-level null, plus spatial reliability and breeding sensitivity.
    chai=next(r for r in e if r['effect_id']=='ECO_PL_PUBIPETALA_FRUIT');y=lnrr(chai)
    aa,nn,cc,mm=(int(chai[k]) for k in ('events_num','n_num','events_den','n_den'))
    se=math.sqrt((1/aa-1/nn)+(1/cc-1/mm))
    ...
    # Climate/season mediation: count independent taxa from admitted study designs, not effect rows.
    med_axes={'climate_pollinator_mediation','climate_phenology_pollination','season_weather_pollination','pollinator_service_weather'}
    med=[r for r in s if r['ecological_axis'] in med_axes and r['admission_status'] in {'admit_primary','admit_mediation'}]
    med_taxa=sorted({r['taxon'] for r in med})
    ...
    abiotic=[r for r in s if r['ecological_axis']=='abiotic_pigment_manipulation']
    sensory=[r for r in e if r['axis']=='sensory_pollinator']
    ...
    with open(a.out_dir/'cross_species_service.csv','w',newline='') as f:
        w=csv.DictWriter(f,fieldnames=['effect_id','taxon','visible_state','RR']);w.writeheader();w.writerows([{'effect_id':r['effect_id'],'taxon':r['taxon'],'visible_state':r['visible_state'],'RR':math.exp(lnrr(r))} for r in cross])
