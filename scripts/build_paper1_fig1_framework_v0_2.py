...
def main():
    ...
    fig,axes=plt.subplots(1,2,figsize=(15,5.8),gridspec_kw={'width_ratios':[1.45,1]})
    ...
    for i,(r,y) in enumerate(zip(fw,ys)):
        ax.text(.05,y,r['title'],fontsize=10,weight='bold',va='center')
        ax.text(.34,y,r['empirical_anchor'],fontsize=8.4,va='center',wrap=True)
        if i<len(fw)-1: ax.annotate('',xy=(.02,y-.08),xytext=(.02,y-.035),arrowprops={'arrowstyle':'->','lw':1.4})
    ax.text(.50,.02,'Each layer is measured separately; downstream outcomes do not fill upstream missing states.',ha='center',fontsize=8.5,weight='bold')
    ...
    ax.text(.02,.98,'Candidate-free protocol:',transform=ax.transAxes,va='top',fontsize=9.5,weight='bold')
    ax.text(.02,.90,'5 systems × 4 axes = 20 canonical cells\n19 resolved; 1 unresolved kept unresolved\n0 significance/expected-direction filters',transform=ax.transAxes,va='top',fontsize=8.8)
    ax.text(.02,.58,'Observed literature recurrence\n≠ biological recurrence by definition',transform=ax.transAxes,fontsize=9,weight='bold')
