...
def fetch(row,outdir):
    tax=row['taxon'];url=row['assembly_url'];dest=outdir/(slug(tax)+'.zip')
    s=requests.Session();s.headers['User-Agent']='Mozilla/5.0 chun-angio353-pilot/0.1'
    last=None
    for attempt in range(3):
        try:
            with s.get(url,stream=True,timeout=(20,240)) as r:
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--panel',type=Path,required=True);ap.add_argument('--out-dir',type=Path,required=True);ap.add_argument('--workers',type=int,default=8);a=ap.parse_args()
    ...
    with ThreadPoolExecutor(max_workers=a.workers) as ex:
        fut={ex.submit(fetch,r,download):r for r in panel}
    ...
    summary={'n_taxa':len(rows),'unique_zip_sha256':len(byhash),'checksum_collisions':collisions,'total_zip_bytes':sum(r['zip_bytes'] for r in rows),'total_fasta_bytes':sum(r['fasta_bytes'] for r in rows),'claim_ceiling':'ID-bound TPIA panel payload provenance; no biological inference'}
