...
def fig2(out:Path):
    ...
    ax.set_ylabel('Same-state phylogenetic AUC')
    ax.set_title('Standardized visible-color profiles across 28 completed clades')
...
def fig5(out:Path):
    d=pd.read_csv(ROOT/'data/flowerclades51_fine_time_persistence_median_curve_v0_1.csv')
    fig,ax=plt.subplots(figsize=(7.4,4.8))
    ax.plot(d['t'],d['median_excess'],marker='o',linewidth=1.5,label='Median excess retention')
    ax.fill_between(d['t'],d['q25'],d['q75'],alpha=0.2,label='Interquartile range')
    ax.axhline(0,linestyle='--',linewidth=1)
    ax.set_xlim(0,1.02)
    ax.set_xlabel('Relative divergence depth')
    ax.set_ylabel('Excess same-color retention')
    ax.set_title('Exact flower-color memory declines with evolutionary divergence')
    ax.legend(frameon=False)
    save(fig,out/'fig5_relative_time_persistence.png')


def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--out-dir',type=Path,default=ROOT/'figures/cross_radiation_el_v0_3_temporal_memory'); a=ap.parse_args()
    for fn in (fig1,fig2,fig3,fig4,fig5): fn(a.out_dir)
    expected=[a.out_dir/f'fig{i}_{name}.png' for i,name in [(1,'iris_profile'),(2,'visible_clades_profiles'),(3,'tree_moderators'),(4,'petunieae_profile'),(5,'relative_time_persistence')]]
