...
def main() -> int:
    ...
    if len(rows) != 16:
        raise SystemExit(f"science v0.2.1 reference registry must contain 16 rows, found {len(rows)}")
    ...
    if len(manuscript_dois) != 16 or set(manuscript_dois) != set(dois):
        raise SystemExit(
    ...
    if "10.1007/s10722-025-02606-6" not in set(dois):
        raise SystemExit("Jiang 2025 CSEMISERRATA DOI missing")

    summary = {
        "status": "paper1_science_reference_registry_v0_2_1_valid",
        "registry_rows": 16,
        "manuscript_reference_dois": 16,
