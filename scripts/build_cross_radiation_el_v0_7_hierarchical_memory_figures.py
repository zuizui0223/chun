...
SYN=ROOT/"data/hierarchical_evolutionary_memory_synthesis_v0_1.json"
...
def figure_specs()->list[dict]:
    return [
        ...
        {"figure":5,"filename":"fig5_evidence_tiers.png",
         "sources":[str(SYN.relative_to(ROOT)),str(RUE.relative_to(ROOT)),str(GATE.relative_to(ROOT))]},
    ]
...
def fig5(out:Path):
    ...
    boxes=[
      (0.50,0.84,"Tier A — standardized visible color\n21 opportunity clades; 18 positive\nmedian hidden effect +0.0270"),
      (0.50,0.62,"Tier B — biochemical Petunieae\nsame conceptual estimator\n+0.1879 AUC; P = 0.0001"),
      (0.50,0.40,"Tier C — external radiations\nLinoideae • Angraecinae • Antirrhineae\n3/3 source-faithful conditional support; not pooled"),
      (0.50,0.17,"Tier D — prospective Ruellia gate\noutcome unopened\nPASS/FAIL pending authoritative tree recovery"),
    ]
    for x,y,t in boxes:
        ax.text(x,y,t,ha="center",va="center",
                bbox=dict(boxstyle="round,pad=0.6",fill=False),fontsize=10)
    for y1,y2 in [(0.76,0.70),(0.54,0.48),(0.32,0.25)]:
        ax.annotate("",xy=(0.5,y2),xytext=(0.5,y1),
                    arrowprops=dict(arrowstyle="->"))
    ax.text(0.02,0.98,"Evidence tiers are deliberately not pooled",ha="left",va="top",fontsize=9)
    ax.text(0.98,0.02,
            f"v0.7 status: {g['status']}\nRuellia: {r['status']}",
            ha="right",va="bottom",fontsize=8)
    ax.set_title("Hierarchical evolutionary memory: evidence tiers and falsification gate",pad=12)
...
def build_all(outdir:Path)->dict:
    ...
    manifest={
        "version":"v0.1",
        "status":"EL_V0_7_HIERARCHICAL_MEMORY_FIGURES_BUILT",
        "candidate":"Evolution Letters v0.7 hierarchical memory",
