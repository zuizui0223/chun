...
def blocked_pair_p(y_t,y_c,rng,nperm):
    pool=np.concatenate([y_t,y_c],axis=1);obs=float(np.mean(y_t.mean(1)-y_c.mean(1)));vals=[]
    batch=300
    for start in range(0,nperm,batch):
        n=min(batch,nperm-start);scores=rng.random((n,15,16));idx=np.argpartition(scores,8,axis=2)[:,:,:8]
        pp=np.broadcast_to(pool,(n,)+pool.shape);tr=np.take_along_axis(pp,idx,axis=2).mean(2);co=(pool.sum(1)[None,:]-tr*8)/8
        vals.extend((tr-co).mean(1).tolist())
...
def blocked_interaction_p(y_tw,y_cw,y_ts,y_cs,rng,nperm):
    pools=[np.concatenate([y_tw,y_cw],1),np.concatenate([y_ts,y_cs],1)]
    obs=float(np.mean((y_tw.mean(1)-y_cw.mean(1))-(y_ts.mean(1)-y_cs.mean(1))));vals=[];batch=300
    for start in range(0,nperm,batch):
...
def sim_pair(delta,reps,nperm,seed):
    rng=np.random.default_rng(seed);plant=blocked=0
    for _ in range(reps):
...
def sim_interaction(winter_delta,summer_delta,reps,nperm,seed):
    rng=np.random.default_rng(seed);plant=blocked=0
    for _ in range(reps):
...
def main():
    ...
    rows=[]
    for j,d in enumerate((0.0,0.1,0.2,0.3)):
        p,b=sim_pair(d,a.reps,a.permutations,20260903+j)
        rows.append({'scenario':'single_season_8v8','winter_delta':d,'summer_delta':'','interaction_delta':'','plant_collapse_rejection':p,'blocked_randomization_rejection':b})
    for j,sd in enumerate((0.0,0.05,0.10,0.15)):
        p,b=sim_interaction(.25,sd,a.reps,a.permutations,20260905+j)
        rows.append({'scenario':'season_interaction_8v8','winter_delta':.25,'summer_delta':sd,'interaction_delta':.25-sd,'plant_collapse_rejection':p,'blocked_randomization_rejection':b})
