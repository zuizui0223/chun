...
def main() -> int:
    ...
    for seed in seeds:
        ...
        for i, ref in enumerate(refs, start=1):
            ...
            back_rows.append({
                "seed_id": sid, "relation": "backward", "ordinal": i, "doi": ref_doi,
                "title_or_unstructured": title, "year": str(ref.get("year") or ""),
        ...
        for rec in fw:
            ...
            forward_rows.append({
                "seed_id": sid, "relation": "forward", "doi": rec["doi"], "title": rec["title"],
                "year": rec["year"], "venue": rec["venue"], "openalex_id": rec["openalex_id"],
                "url": rec["url"],
    ...
    for row in back_rows:
        if row["known_in_repo"] == "true" or row["priority_relevant"] != "true":
            continue
        if row["doi"]:
            candidate_by_doi.setdefault(row["doi"], {
    ...
    for row in forward_rows:
        if row["known_in_repo"] == "true" or row["priority_relevant"] != "true" or not row["doi"]:
            continue
        entry = candidate_by_doi.setdefault(row["doi"], {
    ...
    for doi, row in sorted(candidate_by_doi.items()):
        candidate_rows.append({
            "doi": doi, "title": row["title"], "year": row["year"], "source_relation": row["source_relation"],
            "seed_ids": ";".join(sorted(row["seed_ids"])), "camellia_in_title": row["camellia_in_title"],
            "screen_status": "pending_manual_screen",
        })
    write_csv(args.out_dir / "new_priority_doi_candidates.csv", candidate_rows,
              ["doi", "title", "year", "source_relation", "seed_ids", "camellia_in_title", "screen_status"])
    write_csv(args.out_dir / "new_priority_no_doi_candidates.csv", no_doi_candidates,
