...
UA='chun-petunieae-prospective-source-audit/0.3'
MAX_BYTES=50_000_000
# Root metadata showed large transcriptome/quantification/reconstruction trees. The frozen
# prospective test needs the processed multivariate inputs, trait tables and a source tree,
# which are expected in the analysis/result folders below; raw assemblies are not traversed.
TARGET_ROOTS={
 'additional_raw_data','phyloCCA','phyloPCA','dataset_summary_statistics',
 'stochastic_mapping','SupplementalText'
}
...
def root_targets():
    obj,resolved,_=jget(BASE+'?page[size]=100');rows=[];found=set()
    for item in obj.get('data',[]):
