...
Each genome/version and sequence type is queried separately. Response bytes,
HTTP metadata, FASTA records and checksums are retained. This establishes
sequence provenance but does not itself infer orthology.
"""
...
def main():
    ...
    for col,tea_type in COL_TO_TYPE.items():
        ...
        for seq_type in ['cds','transcript']:
            ...
            resp=s.get(url,timeout=120,allow_redirects=True);raw=resp.content;text=resp.text
            slug=f'{col}_{seq_type}'
            (a.out_dir/f'{slug}.txt').write_bytes(raw)
            records=parse_fasta(text)
            audit.append({'crosswalk_column':col,'tea_type':tea_type,'sequence_type':seq_type,'http_status':resp.status_code,'final_url':resp.url,'content_type':resp.headers.get('content-type',''),'content_disposition':resp.headers.get('content-disposition',''),'response_bytes':len(raw),'response_sha256':sha256(raw),'requested_ids':';'.join(ids),'n_fasta_records':len(records),'body_prefix':text[:200].replace('\n','\\n')})
            for header,seq in records:
                fasta_rows.append({'crosswalk_column':col,'tea_type':tea_type,'sequence_type':seq_type,'header':header,'sequence_length':len(seq),'sequence_sha256':sha256(seq.upper().encode()),'sequence':seq.upper()})
            time.sleep(.2)
