...
def main():
    ...
    # Temporal-memory candidate must carry the frozen retrospective persistence result exactly.
    assert persistence['status']=='FLOWERCLADES51_RELATIVE_TIME_PERSISTENCE_RETROSPECTIVE_RESULT'
    fine_time=persistence['fine_visible_color_all_eligible']
    assert fine_time['eligible_clades']==32
    assert fine_time['slope']['favorable']==23
    assert abs(fine_time['slope']['median'] - (-0.18966775303842393)) < 1e-12
    assert fine_time['slope']['wilcoxon_p_one_sided'] < 0.01
