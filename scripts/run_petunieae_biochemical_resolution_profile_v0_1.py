...
ERRATUM=Path('data/petunieae_biochemical_resolution_profile_source_key_erratum_v0_1.json')
...
def main():
    ...
    e=json.loads(ERRATUM.read_text())
    if p['status']!='FROZEN_BEFORE_PETUNIEAE_BIOCHEMICAL_RESOLUTION_AUC_COMPUTATION': raise ValueError('prereg status drift')
    if e['status']!='FROZEN_POST_SOURCE_IDENTITY_CHECK_PRE_PROFILE_AUC' or e['corrected_source_key']!=OUTGROUP:
        raise ValueError('source-key erratum not frozen')
    if e['profile_auc_computed_before_erratum'] is not False or e['profile_winner_computed_before_erratum'] is not False:
        raise ValueError('erratum timing invalid')
    ...
    if int((df['key_0']==OUTGROUP).sum())!=1: raise ValueError('literal frozen outgroup key absent or duplicated')
    ...
    if len(tips0)!=60 or len(set(tips0))!=60 or set(tips0)!=set(full['key_0']): raise ValueError('source table/tree pre-prune join failure')
    ...
    if reasons:
        out={
          ...
          'source_table_sha256':TABLE_SHA,'source_tree_sha256':TREE_SHA,'source_key_erratum_status':e['status'],
          ...
          'resolution_profile_AUC_computed':False,'winner_computed':False,
          'do_not_count_as_new_prospective_replication':True,'paper1_science_changed':False,
        }
    ...
    out={
      ...
      'source_key_erratum_status':e['status'],'source_defined_outgroup_key_used':OUTGROUP,
