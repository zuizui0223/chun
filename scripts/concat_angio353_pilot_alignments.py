...
def main():
    ...
    a.summary.write_text(json.dumps({'n_taxa':len(tips),'n_loci':len(used),'alignment_aa':len(next(iter(concat.values()))),'min_nongap':a.min_nongap,'loci':used,'claim_ceiling':'pilot concatenated AA matrix from best Angiosperms353 HSPs'},indent=2)+'\n')
