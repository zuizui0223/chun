...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--out-dir',type=Path,required=True);ap.add_argument('--scenario',choices=['sensory_plus_reward','reward_only','latent_unresolved'],required=True);a=ap.parse_args();a.out_dir.mkdir(parents=True,exist_ok=True)
    rows=[]
    for i in range(1,16):
        ...
        for season in ('summer','winter'):
            ...
            bird_share=1/(1+math.exp(-linear));service=total_service+0.35*w
