...
Our aim is therefore broader than showing that character coding affects phylogenetic inference. We test whether flower-color states retain a repeatable signature of evolutionary memory, whether that memory decays with relative divergence, and whether one phenotypic resolution consistently preserves it best. The resulting evidence supports temporal persistence of exact flower-color states while rejecting a universal privileged resolution, shifting the next question toward the ecological and mechanistic causes of between-clade differences in memory loss.
...
Flower-color evolution is neither memoryless nor governed by one universally privileged phenotypic scale. Across the standardized source phylogenies, exact visible flower-color states generally showed greater-than-baseline similarity among relatively recent divergences, and that excess retention declined toward deeper relative divergence. At the same time, the representation in which phylogenetic structure was strongest varied among clades. These two results distinguish the existence of evolutionary memory from the phenotypic scale at which that memory is most visible.
