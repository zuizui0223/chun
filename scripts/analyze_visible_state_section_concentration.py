...
def main():
    ...
    rows = [r for r in rows if r.get("colour_state") in {"A", "W", "Y"} and normalize_section(r.get("section", ""))]
    ...
    for j, state in enumerate(state_levels):
        o = observed[state]
        lower_breadth = (int(np.sum(breadth[:, j] <= o["observed_section_breadth"])) + 1) / (b + 1)
        lower_entropy = (int(np.sum(entropy[:, j] <= o["observed_section_entropy"] + 1e-12)) + 1) / (b + 1)
        out.append({
            "visible_state": state,
            "n_species": o["n_species"],
            "n_observed_sections_total": len(section_levels),
            "observed_section_breadth": o["observed_section_breadth"],
            "expected_section_breadth_under_label_shuffle": float(breadth[:, j].mean()),
            "lower_tail_breadth_p": lower_breadth,
            "observed_section_entropy": o["observed_section_entropy"],
            "expected_section_entropy_under_label_shuffle": float(entropy[:, j].mean()),
            "lower_tail_entropy_p": lower_entropy,
            "permutations": b,
            "seed": args.seed,
            "interpretation": (
                "state is more section-concentrated than expected for its species count"
                if lower_breadth < 0.05 and lower_entropy < 0.05
                else "no count-controlled evidence of unusually low section breadth/entropy"
            ),
        })
