...
**Flower-color evolutionary memory is hierarchically retained across phenotype scales**

Running title: Hierarchical retention of flower-color memory

## Teaser text

Evolutionary history can be retained simultaneously at nested phenotype scales. Across standardized flower-color clades, finer identities often remain phylogenetically organized inside coarser states, and this hidden-memory pattern prospectively replicated in an independently admitted *Rhododendron* sect. *Schistanthe* radiation. The result explains why no single coarse, intermediate, or fine representation must win globally.

# Abstract

Phenotypes can preserve evolutionary history at several nested biological scales, so asking which single representation is most predictable may miss structure stored inside broader states. Flower color provides a test because pigmentation, visible color identity, and biochemical composition form natural hierarchies. A preregistered intermediate-resolution optimum failed in an independent *Iris* radiation, and a standardized 51-clade analysis yielded heterogeneous coarse, fine, tied, and no-signal profiles. Reparameterizing the opened profiles showed that overall memory amplitude explained 90.65% of between-clade variation, whereas coarse-to-fine scale tilt explained 6.49% and intermediate-specific curvature 2.86%. We then conditioned on coarse state membership and asked whether fine identity retained additional phylogenetic organization. Among 21 standardized clades with genuine fine-to-coarse opportunity, 18 had positive permutation-centered hidden-memory effects; the median was +0.0270 AUC (one-sided Wilcoxon P = 3.34 × 10^-5; sign-test P = 7.45 × 10^-4). Retrospective biochemical Petunieae showed a +0.1879 AUC hidden-memory effect (9,999-permutation P = 0.0001), and three external radiations independently support conditional fine-state organization under nonexchangeable estimators. Crucially, an outcome-blind held-out *Rhododendron* sect. *Schistanthe* radiation prospectively replicated the prediction: conditional AUC = 0.5965 versus a within-coarse null mean of 0.5323, centered effect +0.0642, P = 0.0033. Thus flower-color history is repeatedly retained hierarchically: coarse states can preserve broad lineage history while finer identities retain additional organization within them. A prospectively frozen biochemical *Ruellia* test remains unopened, so cross-representation prospective generalization is still bounded.
...
We evaluate the resulting architecture in deliberately separated evidence tiers. The standardized visible-color batch supplies the broadest same-estimand evidence. Petunieae provides a biochemical cross-representation application of the same conditional AUC logic. Three external radiations provide independent qualitative replication under source-faithful conditional hierarchy tests that are not pooled statistically with the standardized batch. We then test the architecture prospectively in an independently admitted *Rhododendron* sect. *Schistanthe* radiation whose source files, identifier crosswalk, and state-support opportunity were frozen sequentially before its phylogenetic outcome was opened. Finally, an outcome-unopened biochemical *Ruellia* test remains the precommitted cross-representation falsification gate. Our goal is therefore not to declare a universal hierarchical-memory law, but to determine whether the negative search for one optimal resolution conceals a positive, nested organization of evolutionary history that survives prospective testing.
...
## Prospective *Schistanthe* validation

We searched for an additional held-out radiation under admission rules frozen before target outcomes were inspected. *Rhododendron* sect. *Schistanthe* was admitted because an exact public flower-color file and a branch-length chronogram were available. The Dryad object identities were frozen before file recovery, and recovered bytes were required to match the published SHA-256 digests. An identifier-only crosswalk was then frozen before flower-color values were opened: 129 of 130 trait identifiers matched tree tips exactly; the one unmatched trait row was excluded without alias or fuzzy repair.

Only after this crosswalk freeze was the target color column opened for support counts. The retained frame contained red = 63, white = 36, and yellow = 30, all above the predeclared minimum support of five. The deterministic hierarchy was fine = exact source color and coarse = WHITE versus NONWHITE, giving 3 fine states and 2 coarse states and therefore genuine compression opportunity.

The prospective hidden-memory test then followed the pre-frozen conditional estimator: retain same-coarse pairs, score them by negative patristic distance, classify same-fine identity, and compare observed conditional AUC with 9,999 permutations of fine labels within coarse groups using seed 20260920. PASS required observed conditional AUC to exceed the permutation mean and one-sided P <= 0.05.
...
## Independent prospective *Schistanthe* validation passed

The held-out *Rhododendron* sect. *Schistanthe* frame retained 129 exact trait–tree matches. Before the phylogenetic outcome was opened, state support and compression opportunity were frozen at red = 63, white = 36, yellow = 30 and coarse NONWHITE = 93, WHITE = 36.

Among pairs already sharing the same coarse state, the observed conditional fine-color AUC was **0.5965**. The mean of 9,999 within-coarse permutations was **0.5323**, giving a centered hidden-memory effect of **+0.0642 AUC**. The null 95% interval was 0.4990–0.5768 and the one-sided permutation test gave **P = 0.0033**. The predeclared PASS criterion was therefore met.

This is an independent prospective validation of hidden fine memory in a visible-color radiation. It was not used to redefine the earlier Flower-clades estimator or the pre-frozen biochemical *Ruellia* promotion gate.

## Prospective biochemical cross-representation validation remains unresolved

The architecture has now prospectively replicated in the independent visible-color *Schistanthe* radiation, but prospective biochemical generalization remains unresolved. The outcome-unopened *Ruellia* system carries a separately frozen conditional prediction. If its retained HPLC frame contains fine-to-coarse compression opportunity, fine compound states are predicted to retain positive within-coarse hidden memory under the same 9,999-permutation logic. Row-level HPLC states, frequencies, and AUCs remain unopened. The blocker is recovery of the authoritative tree used by the source analysis. The current evidence therefore supports a prospectively validated hierarchical architecture in visible flower color plus retrospective biochemical concordance, not a universal cross-representation law.
...
The Petunieae result is especially informative because it changes phenotype representation while retaining the conceptual question. Its +0.1879 conditional AUC effect indicates that exact compound identity remains organized inside a coarse pigment-presence state. The independent *Schistanthe* test adds a different strength: it prospectively validates the hidden-memory architecture in a radiation that was admitted and gated before its target color outcomes were opened. Together, these results move the programme beyond a retrospective synthesis, while still stopping short of prospective biochemical generalization.

The prospective *Ruellia* gate now has a narrower but still important role. *Schistanthe* already supplies independent prospective validation of the architecture in visible color. *Ruellia* asks whether that architecture prospectively crosses representation type into biochemical composition. Its HPLC hierarchy was frozen before row-level compound states were opened, and the hidden-memory prediction remains unavailable pending authoritative tree recovery. A PASS would supply held-out biochemical generalization; a FAIL would bound the architecture to the currently supported representations rather than erase the prospective *Schistanthe* result. The pre-frozen v0.7 promotion rule is not rewritten after the favorable *Schistanthe* outcome; **the current submission remains v0.3**.
...
There are important limitations. The standardized 21-clade hidden-memory result is exploratory and post-outcome, even though the estimator is fixed and source-exact. Petunieae is retrospective. The three external radiations use nonexchangeable estimators and cannot be pooled numerically. *Schistanthe* is prospectively clean but tests visible color rather than biochemical composition. Hidden-memory AUC measures same-state phylogenetic organization, not ancestral polymorphism duration, transition direction, or absolute time, and it does not identify one causal mechanism. Finally, the presently blocked source systems show that a phenotype hierarchy must be both biologically meaningful and supported by sufficient replicated state counts before this framework can be applied.

The strongest current conclusion is positive but bounded. Flower-color history is not well described by one scale-optimal representation. Across multiple radiations, fine phenotype identity often preserves additional historical organization after coarse state membership is fixed, while the amount of retained history and the globally dominant scale vary separately. This architecture now has independent prospective support in *Schistanthe*. Evolutionary memory in flower color is therefore better viewed as hierarchically retained across phenotype scales than as a contest among coarse, intermediate, and fine encodings. Prospective biochemical falsification remains the next test.

# Data and code availability

All analysis contracts, exact-source manifests, machine-readable results, validation workflows, and evidence-tier syntheses are versioned in the `zuizui0223/chun` repository. The standardized 51-clade source data are archived in Dryad under DOI 10.5061/dryad.r4xgxd2sc. Petunieae processed data and code are archived at OSF node `zg9cu`. The prospective *Schistanthe* source is archived in Dryad under DOI 10.5061/dryad.47d7wm3f4; exact trait and chronogram bytes were verified against frozen SHA-256 digests. The outcome-unopened *Ruellia* source is deposited in Figshare under DOI 10.6084/m9.figshare.30282448; its authoritative author-used tree remains the current biochemical source-provenance blocker. A submission-specific repository snapshot would be assigned a persistent archive identifier before any promotion of this candidate.
...
**Figure 5. Evidence tiers and prospective falsification gates.** Standardized visible-color evidence, retrospective biochemical Petunieae evidence, three non-pooled external conditional-hierarchy radiations, the prospective *Schistanthe* PASS, and the outcome-unopened biochemical *Ruellia* test are shown as separate tiers. **Alt text:** Evidence tiers progress from standardized retrospective prevalence through biochemical and external concordance to a highlighted prospective Schistanthe PASS and a final pending prospective biochemical Ruellia gate.
...
Soza, V. L., Kriebel, R., Ramage, E., Hall, B. D., & Twyford, A. D. (2022). The symmetry spectrum in a hybridising, tropical group of rhododendrons. *New Phytologist, 234*, 1491–1506. https://doi.org/10.1111/nph.18083
