...
def fetch(row,outdir,read_timeout,retries,wall_timeout):
    ...
    s=requests.Session();s.headers['User-Agent']='Mozilla/5.0 chun-angio353-pilot/0.4'
    last=None
    for attempt in range(1,retries+1):
        ...
        try:
                ...
                with dest.open('wb') as f:
                    for chunk in r.iter_content(1024*1024):
                        ...
                        if elapsed>wall_timeout:
                            raise TimeoutError(f'wall-clock download timeout after {elapsed:.1f}s; bytes={n}')
            ...
            return {'taxon':tax,'tip':slug(tax),'colour_state':row['colour_state'],'section':row['section'],'tpia_id':row['tpia_id'],'resource_name':row['tpia_resource_name'],'url':url,'zip_file':dest.name,'zip_bytes':n,'zip_sha256':h.hexdigest(),'download_seconds':round(elapsed,3),'inner_member':info.filename,'status':'ok'}
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--panel',type=Path,required=True);ap.add_argument('--out-dir',type=Path,required=True);ap.add_argument('--workers',type=int,default=8);ap.add_argument('--read-timeout',type=int,default=60);ap.add_argument('--wall-timeout',type=int,default=300);ap.add_argument('--retries',type=int,default=2);a=ap.parse_args()
    ...
    summary={'n_panel_rows':len(all_panel),'n_admitted_taxa':len(rows),'n_quarantined_taxa':len(quarantine),'quarantined_taxa':[r.get('taxon','') for r in quarantine],'unique_zip_sha256':len(byhash),'checksum_collisions':collisions,'total_zip_bytes':sum(r['zip_bytes'] for r in rows),'total_fasta_bytes':sum(r['fasta_bytes'] for r in rows),'read_timeout_seconds':a.read_timeout,'wall_timeout_seconds':a.wall_timeout,'retries':a.retries,'claim_ceiling':'provenance-screened admitted ID-bound TPIA panel payloads; quarantined taxa excluded; no biological inference'}
